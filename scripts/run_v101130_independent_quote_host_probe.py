...
rows=[]
add(rows,'release_identity',"const APP_VERSION = 'v101.130';" in html and "const APP_EVIDENCE_STAGE = 'FOUR_PASS_FINAL_PACKAGE_METADATA_EVIDENCE_RECONCILIATION_R1';" in html)
...
obj={'schema':'L24H_V101130_INDEPENDENT_QUOTE_HOST_PROBE_V1','version':'v101.130','implementation':'Independent probe; reads candidate bytes only and does not read primary v101.130 matrix outputs or the mutation ledger.','summary':{'pass':sum(r['status']=='PASS' for r in rows),'fail':sum(r['status']=='FAIL' for r in rows),'total':len(rows)},'rows':rows}
