...
HTML=Path(sys.argv[1]).read_text(encoding='utf-8'); OUT=Path(sys.argv[2]); VERSION='v101.120'; STAGE='HELP_INFORMATION_ARCHITECTURE_AND_ATTRIBUTION_CLARITY_R1'
profiles=[
...
async def main():
 ...
 OUT.write_text(json.dumps({'schema':'L24H_V101120_HELP_BROWSER_MATRIX_V1','version':VERSION,'browser':'system Chromium via Playwright content injection','real_device_limitation':'Not physical Safari/Samsung/TalkBack/VoiceOver evidence','summary':summary,'results':rows},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
