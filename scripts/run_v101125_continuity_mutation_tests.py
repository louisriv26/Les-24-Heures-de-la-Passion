...
out={'schema':'L24H_V101125_CONTINUITY_MUTATION_TESTS_V1','version':'v101.125','baseline_validator_status':'PASS' if base_ok else 'FAIL','baseline_checks':base_checks,'summary':{'pass':sum(r['status']=='PASS' for r in rows),'fail':sum(r['status']=='FAIL' for r in rows),'total':len(rows)},'rows':rows}
