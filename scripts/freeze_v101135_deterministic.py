...
def freeze(root,out):
 ...
 with zipfile.ZipFile(out,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=9,strict_timestamps=True) as z:
  for p in paths:
   rel=p.relative_to(root).as_posix();zi=zipfile.ZipInfo(rel,date_time=FIXED_DT);zi.compress_type=zipfile.ZIP_DEFLATED;zi.create_system=3;zi.external_attr=(0o100644<<16);zi.flag_bits|=0x800
   z.writestr(zi,p.read_bytes(),compress_type=zipfile.ZIP_DEFLATED,compresslevel=9)
 with zipfile.ZipFile(out) as z:
  if z.testzip() is not None:raise RuntimeError('zip corruption')
  n=len(z.infolist())
 return {'zip':str(out),'sha256':sha(out),'members':n,'bytes':out.stat().st_size}
