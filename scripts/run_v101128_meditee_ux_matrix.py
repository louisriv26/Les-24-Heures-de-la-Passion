...
HTML=Path(sys.argv[1]).read_text(encoding='utf-8'); OUT=Path(sys.argv[2]); VERSION='v101.128'
PROFILES=[
...
async def main():
 ...
 async with async_playwright() as pw:
  ...
  print('GROUP responsive',flush=True)
  # Responsive/accessibility matrix: five profiles, light/dark, normal/large, read/unread.
   ...
   for scheme in ['light','dark']:
    for font in ['normal','large']:
     for isread in [False,True]:
      ...
      key=f'{label}_{scheme}_{font}_{"read" if isread else "unread"}'
      add('responsive',key+'_identity',d['v']==VERSION and d['theme']==scheme and d['font']==font,d)
      add('responsive',key+'_no_overflow',d['bodyW']<=d['inner']+2 and d['contentW']<=d['contentClient']+2 and d['barL']>=-2 and d['barR']<=d['inner']+2,d)
      add('responsive',key+'_touch_targets',d['topH']>=44 and d['botH']>=40,d)
      add('responsive',key+'_aria_truth',d['topAria']==('true' if isread else 'false') and d['botAria']==('true' if isread else 'false') and bool(d['topLabel']) and bool(d['botLabel']),d)
      add('responsive',key+'_normal_flow',d['pos'] not in ('fixed','sticky'),d['pos'])
      add('responsive',key+'_no_duplicate_ids',len(d['dups'])==0,d['dups'])
   add('responsive',f'{label}_no_page_errors',not errs,errs);await ctx.close()
  await b.close()
 summary={'pass':sum(r['status']=='PASS' for r in rows),'fail':sum(r['status']=='FAIL' for r in rows),'total':len(rows)}
 OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps({'schema':'L24H_V101128_MEDITEE_UX_MATRIX_V1','version':VERSION,'real_device_limitation':'Chromium/browser profile evidence only; physical iPhone/iPad/Samsung and VoiceOver/TalkBack remain open','summary':summary,'rows':rows},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
