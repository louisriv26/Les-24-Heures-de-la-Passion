...
BASE_ZIP=Path('/mnt/data/L24H_v101116_GITHUB_DEPLOY_FOUR_PASS_SELF_CONTAINED_BUILD_INTEGRITY_REPAIR_R1_LOCKED.zip')
BASE_SHA='2ed387c56530d58e54852e748d00aaa425513dfc626b516e53a3c4ef91d356f5'
VERSION='v101.117';CACHE='luisa-24h-v101-117';DATE='2026-08-25';STAGE='FOUR_PASS_SEMANTIC_CURRENT_METADATA_INTEGRITY_REPAIR_R1'
OUT=Path('/mnt/data/L24H_v101117_BUILD');FINAL=Path('/mnt/data/L24H_v101117_GITHUB_DEPLOY_FOUR_PASS_SEMANTIC_CURRENT_METADATA_INTEGRITY_REPAIR_R1_LOCKED.zip')
...
html=base_html.replace("const APP_VERSION = 'v101.116';","const APP_VERSION = 'v101.117';",1).replace("const APP_EVIDENCE_STAGE = 'FOUR_PASS_SELF_CONTAINED_BUILD_INTEGRITY_REPAIR_R1';",f"const APP_EVIDENCE_STAGE = '{STAGE}';",1)
old="const BUILD_DATE = '2026-08-25'; // v101.116 / self-contained build integrity repair";new="const BUILD_DATE = '2026-08-25'; // v101.117 / semantic current-metadata integrity repair";assert old in html;html=html.replace(old,new,1)
for n in PROT:assert ex(html,n)==BD[n],n
HTML_SHA=shab(html.encode())
qa=(BASE/'REAL_DEVICE_QA_CHECKLIST.md').read_text(encoding='utf-8').replace('v101.116','v101.117').replace('FOUR_PASS_SELF_CONTAINED_BUILD_INTEGRITY_REPAIR_R1',STAGE).replace('luisa-24h-v101-116',CACHE)
...
def prepare(t):
 ...
 wt(t/'README.md',f'''# Les 24 Heures de la Passion — {VERSION}\n\nStage: `{STAGE}`\n\nThis is a narrow release/evidence-integrity successor to immutable v101.116. The four-pass audit found no governed runtime/corpus/speaker/flow defect, but found three current-facing semantic metadata/report contradictions: stale scope authority wording inherited from RA19E.2, a stale `version.json.release_scope` inherited from v101.115, and a current no-regression ledger that omitted the v101.116 self-contained-build repair. v101.117 corrects those evidence artifacts and parameterizes the current independent prefreeze runner so it always reads the evidence folder for the requested version.\n\nNo governed corpus, text, display, topology, RA19E.2 speaker/presentation or RA19B flow declaration changes. The Hour-3/Hour-22 fixes and all v101.116 runtime behaviour are inherited byte-for-byte at governed declaration level.\n\nThe line-audit CSV is the sole self-excluded audit output; it is independently checked after ZIP freeze. Final reopened-ZIP evidence remains external. Physical devices, live origin, installed-PWA/offline and representative screen-reader gates remain external.\n''')
 ...
 v=json.loads((t/'version.json').read_text());v.update({'app_version':VERSION,'build_date':DATE,'cache_name':CACHE,'release_scope':'Four-pass semantic current-metadata/report-integrity repair: correct scope authority, release-scope description, current no-regression ledger, and version-parameterized prefreeze evidence lookup; no governed runtime declaration changes.','real_device_status':f'Physical Samsung/iPhone/iPad and live-origin PWA/offline/accessibility validation NOT_TESTED for {VERSION}.','overall_release_status':'LIMITED_PASS_STATIC_IF_EXTERNAL_FINAL_REOPEN_GATES_PASS','known_blockers':[]});wt(t/'version.json',json.dumps(v,ensure_ascii=False,indent=2)+'\n')
 m=json.loads((t/'manifest.json').read_text());m['version']=VERSION;wt(t/'manifest.json',json.dumps(m,ensure_ascii=False,indent=2)+'\n')
 sw=(t/'sw.js').read_text().replace('/* v101.116 */','/* v101.117 */',1).replace("const CACHE_NAME = 'luisa-24h-v101-116';",f"const CACHE_NAME = '{CACHE}';",1);wt(t/'sw.js',sw)
 prov={'version':VERSION,'build_date':DATE,'stage':STAGE,'baseline_version':'v101.116','baseline_zip_sha256':BASE_SHA,'baseline_html_sha256':BASE_HTML_SHA,'candidate_html_sha256':HTML_SHA,'scope':{'governed_runtime_declarations_changed':0,'semantic_current_metadata_repairs':3,'independent_prefreeze_runner_parameterized':True,'active_source_reports_written_before_line_audit':True,'active_report_line_audit_self_exclusion':['reports/active_report_line_audit.csv'],'qa_template_scenarios':21,'build_runner_dependencies':'SELF_CONTAINED_SIBLING_SCRIPTS'},'final_reopen_evidence':'EXTERNAL_AFTER_ZIP_FREEZE'};wt(t/'metadata/build_provenance.json',json.dumps(prov,ensure_ascii=False,indent=2)+'\n')
 wt(t/'metadata/scope_escalation_authority.md',f'''# Scope authority — {VERSION}\n\nCurrent stage: `{STAGE}`.\n\nThis successor is restricted to release/evidence metadata and build-audit machinery: correcting stale current scope/release wording, recording the v101.116 repair in the active no-regression ledger, parameterizing current prefreeze evidence lookup, propagating release identity, and regenerating evidence/manifests.\n\nNo governed runtime declaration, canonical devotional text, RA19B source-flow decision, RA19E.2 semantic/presentation decision, feature behaviour or UX behaviour is authorised to change in this stage. The inherited v101.112 Hour-3/Hour-22 corrections and v101.111 RA19E.2 ten-action reconciliation remain protected. Any future corpus/flow/speaker/UX mutation requires a new explicitly authorised scope.\n''')
 ev=t/'evidence/v101117';ev.mkdir(parents=True,exist_ok=True)
 ...
 wt(t/'reports/current_metadata_semantic_consistency.md',f'''# Current metadata semantic consistency — {VERSION}\n\nStatus: `PASS_PREPACKAGE`.\n\n- `version.json.release_scope` describes `{STAGE}` and does not describe the superseded v101.115 line-closure stage.\n- `metadata/scope_escalation_authority.md` describes the current release/evidence-only scope and does not present the old RA19E.2 ten-action mutation as this package's current mutation scope.\n- `reports/no_regression_fix_ledger.csv` contains the four AUD-117 repairs plus the v101.116 self-contained-build repair as inherited protected history.\n- `scripts/run_independent_prefreeze_audit.py` derives `evidence/<version-without-dot>/` from its requested version; it does not hard-code `evidence/v101116`.\n- Governed runtime declarations remain exact v101.116 parity.\n''')
 wt(t/'reports/report_claims_vs_evidence_audit.md',f'''# Report claims vs evidence — {VERSION}\n\nThe current builder and three audit runners are packaged sibling scripts. Current metadata semantic consistency is checked explicitly in `reports/current_metadata_semantic_consistency.md` and by the stale/semantic scanner. The active source-report inventory is stored in `metadata/active_report_inventory.json`. Every nonblank line of every listed source report is represented in `reports/active_report_line_audit.csv`, generated only after all listed reports exist. The audit CSV is the sole self-excluded audit output and is checked independently after ZIP freeze. Broad Chromium and isolated service-worker PASS claims bind to packaged prefreeze JSON evidence. Physical-device, live-origin PWA/offline and screen-reader tests remain `NOT_TESTED`; final reopened-ZIP PASS is not claimed inside the package.\n''')
 # Current scripts and runners.
 shutil.copy2(RUNTIME,t/'scripts/run_broad_runtime_matrix.py');shutil.copy2(SWRUN,t/'scripts/run_sw_logic_matrix.js');shutil.copy2(INDEP,t/'scripts/run_independent_prefreeze_audit.py');shutil.copy2(Path(__file__),t/'scripts/build_v101117_semantic_metadata_integrity_repair.py')
...
for t in (A,B):
 ...
 R=json.loads((t/'evidence/v101117/BROAD_CHROMIUM_RUNTIME_MATRIX.json').read_text());S=json.loads((t/'evidence/v101117/SERVICE_WORKER_LOGIC_MATRIX.json').read_text());assert R['summary']=={'pass':52,'fail':0,'total':52};assert S['summary']=={'pass':15,'fail':0,'total':15}
 # exact protected parity baseline
 h=(t/'index.html').read_text();assert all(ex(h,n)==BD[n] for n in PROT)

matrix=f'''gate,test,status,evidence\nBASELINE,v101.116 exact SHA/member freeze,PASS,{BASE_SHA}\nBUILD,deterministic A/B full-tree build,PASS,asserted before ZIP freeze\nBUILD-DEPS,current build runner dependencies,PASS,self-contained packaged sibling scripts; no obsolete external runner paths\nDATA,14 governed runtime declarations,PASS,exact RHS byte parity with v101.116\nSEMANTIC-META,current scope/release/ledger/runner semantics,PASS,reports/current_metadata_semantic_consistency.md\nH3-H22,user-confirmed text/paragraph repairs preserved,PASS,broad runtime 4/4 profiles\nQA-META,root physical/live QA metadata,PASS,21/21 template rows target {VERSION}\nJS,inline JavaScript syntax,PASS,reports/javascript_syntax_check.json\nSW-SYNTAX,service-worker syntax,PASS,reports/service_worker_syntax_check.json\nRUNTIME,broad Chromium DOM/runtime matrix,PASS,evidence/v101117/BROAD_CHROMIUM_RUNTIME_MATRIX.json 52/52\nSW-LOGIC,isolated service-worker logic,PASS,evidence/v101117/SERVICE_WORKER_LOGIC_MATRIX.json 15/15\nACTIVE-REPORTS,all inventoried active source-report lines,PASS,reports/active_report_line_audit.csv exact coverage\nSTALE,current-facing token + semantic stale scan,PASS,reports/stale_reference_scan.* failures 0\nphysical devices,iPhone/iPad/Samsung,NOT_TESTED,external\nlive PWA/offline,real origin/service worker,NOT_TESTED,external\nscreen reader,VoiceOver/TalkBack,NOT_TESTED,external\n'''
for t in (A,B):wt(t/'reports/full_regression_matrix.csv',matrix)

ACTIVE={'README.md','REAL_DEVICE_QA_CHECKLIST.md','REAL_DEVICE_QA_RESULTS_TEMPLATE.csv','version.json','manifest.json','sw.js','metadata/build_provenance.json','metadata/scope_escalation_authority.md','evidence/v101117/PREPACKAGE_STAGE_REPORT.md','reports/full_regression_matrix.csv','reports/no_regression_fix_ledger.csv','reports/current_metadata_semantic_consistency.md','reports/report_claims_vs_evidence_audit.md','reports/root_deploy_consistency_report.md','reports/nested_zip_consistency_report.md','reports/javascript_syntax_check.json','reports/service_worker_syntax_check.json'}
CURRENT_SCRIPTS={'scripts/build_v101117_semantic_metadata_integrity_repair.py','scripts/run_broad_runtime_matrix.py','scripts/run_sw_logic_matrix.js','scripts/run_independent_prefreeze_audit.py'}

def allow(rel,tok):
 if tok in {'v101.116','L24H_v101116_'}:return rel in {'README.md','metadata/build_provenance.json','metadata/scope_escalation_authority.md','evidence/v101117/PREPACKAGE_STAGE_REPORT.md','reports/full_regression_matrix.csv','reports/no_regression_fix_ledger.csv','reports/current_metadata_semantic_consistency.md','scripts/build_v101117_semantic_metadata_integrity_repair.py','scripts/run_independent_prefreeze_audit.py'}
 if tok=='v101.115':return rel in {'README.md','reports/no_regression_fix_ledger.csv','reports/current_metadata_semantic_consistency.md'}
 if tok in {'v101.114','v101.113'}:return rel=='reports/no_regression_fix_ledger.csv'
 if tok in {'v101.112','v101.111'}:return rel in {'reports/no_regression_fix_ledger.csv','metadata/scope_escalation_authority.md'}
 return False

def semantic_checks(t):
 failures=[]
 v=json.loads((t/'version.json').read_text())
 if 'semantic current-metadata' not in v.get('release_scope','').lower(): failures.append(['version.json.release_scope','CURRENT_SCOPE_NOT_SEMANTIC_METADATA_STAGE'])
 if 'active-report line-audit closure' in v.get('release_scope','').lower(): failures.append(['version.json.release_scope','STALE_V101115_SCOPE_PHRASE'])
 ...
 if 'This package is restricted to the exact ten-action RA19E.2 speaker/presentation ledger plus release-shell version propagation.' in sc: failures.append(['metadata/scope_escalation_authority.md','STALE_RA19E2_SCOPE'])
 led=(t/'reports/no_regression_fix_ledger.csv').read_text()
 for aid in ['AUD-117-001','AUD-117-002','AUD-117-003','AUD-117-004','AUD-116-INHERITED']:
 ...
 if "evidence/v101116" in indep: failures.append(['scripts/run_independent_prefreeze_audit.py','HARDCODED_V101116_EVIDENCE'])
...
def stale(t):
 toks=['v101.116','v101.115','v101.114','v101.113','v101.112','v101.111','luisa-24h-v101-116','luisa-24h-v101-115','luisa-24h-v101-114','luisa-24h-v101-113','luisa-24h-v101-112','luisa-24h-v101-111','L24H_v101116_','L24H_v101115_','L24H_v101114_','L24H_v101113_','L24H_v101112_','L24H_v101111_'];rows=[];fail=0
 ...
 for p in fs(t):
  ...
  if rel=='scripts/build_v101117_semantic_metadata_integrity_repair.py':continue
  for tok in toks:
...
def lineaudit(t):
 R=json.loads((t/'evidence/v101117/BROAD_CHROMIUM_RUNTIME_MATRIX.json').read_text());S=json.loads((t/'evidence/v101117/SERVICE_WORKER_LOGIC_MATRIX.json').read_text())
 rows=[]
 for rel in ACTIVE_REPORTS:
  ...
  for ln,line in enumerate(p.read_text(encoding='utf-8',errors='ignore').splitlines(),1):
   ...
   if 'v101.116' in s:typ='CLAIM';bind='BASELINE_OR_INHERITED_PROVENANCE'
   for vv in ['v101.115','v101.114','v101.113','v101.112','v101.111']:
    if vv in s:
     allowed_line = (rel=='reports/no_regression_fix_ledger.csv' or (vv=='v101.115' and rel in {'README.md','reports/current_metadata_semantic_consistency.md'}) or (vv in {'v101.112','v101.111'} and rel=='metadata/scope_escalation_authority.md'))
