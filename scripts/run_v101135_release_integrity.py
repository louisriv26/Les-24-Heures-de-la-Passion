...
STAGE='MASTER_SCRIPT_VALIDATION_COMPLETION_R1'
...
add(rs,'baseline_identity',"const APP_VERSION = 'v101.134';" in b and "const APP_EVIDENCE_STAGE = 'BOUNDARY_UNIVERSE_EVIDENCE_REPRODUCIBILITY_RECONCILIATION_R1';" in b)
add(rs,'candidate_identity',"const APP_VERSION = 'v101.135';" in c and f"const APP_EVIDENCE_STAGE = '{STAGE}';" in c)
def norm(s,base):
 if base:
  s=s.replace("const APP_VERSION = 'v101.134';","const APP_VERSION = '__VERSION__';",1)
  s=s.replace("const APP_EVIDENCE_STAGE = 'BOUNDARY_UNIVERSE_EVIDENCE_REPRODUCIBILITY_RECONCILIATION_R1';","const APP_EVIDENCE_STAGE = '__STAGE__';",1)
  ...
  s=s.replace("const APP_VERSION = 'v101.135';","const APP_VERSION = '__VERSION__';",1)
  s=s.replace(f"const APP_EVIDENCE_STAGE = '{STAGE}';","const APP_EVIDENCE_STAGE = '__STAGE__';",1)
  s=s.replace("const BUILD_DATE = '2026-09-04'; // v101.135 / master-script validation completion; v101.134 functional renderer behavior unchanged","const BUILD_DATE = '__DATE__'; // __RELEASE_COMMENT__",1)
 return s
...
add(rs,'storage_schema_unchanged',bv['storage_schema']==cv['storage_schema'])
add(rs,'personal_snapshot_unchanged',bv['personal_snapshot']==cv['personal_snapshot'])
...
add(rs,'release_scope_declared_nonfunctional',cv.get('release_scope','').startswith('Validation/release-engineering-only successor'))
add(rs,'external_pwa_predecessor_correct','installed PWA update from v101.132' in cv.get('external_open_gates',[]),cv.get('external_open_gates'))
sm={'pass':sum(x['status']=='PASS' for x in rs),'fail':sum(x['status']=='FAIL' for x in rs),'total':len(rs)}
OUT.write_text(json.dumps({'schema':'L24H_V101135_RELEASE_INTEGRITY_V1','version':'v101.135','summary':sm,'rows':rs},ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps(sm));raise SystemExit(2 if sm['fail'] else 0)
