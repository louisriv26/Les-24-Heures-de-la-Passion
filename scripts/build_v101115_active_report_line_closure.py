...
BASE_ZIP=Path('/mnt/data/L24H_v101114_GITHUB_DEPLOY_FOUR_PASS_AUDIT_EVIDENCE_INTEGRITY_REPAIR_R1_LOCKED.zip')
BASE_SHA='80cba7c21fed8c0d3d43b73dc3f502e594363ccf64a23f0fb40ffb5f9f11716b'
VERSION='v101.115';CACHE='luisa-24h-v101-115';DATE='2026-08-25';STAGE='FOUR_PASS_ACTIVE_REPORT_LINE_CLOSURE_R1'
OUT=Path('/mnt/data/L24H_v101115_BUILD');FINAL=Path('/mnt/data/L24H_v101115_GITHUB_DEPLOY_FOUR_PASS_ACTIVE_REPORT_LINE_CLOSURE_R1_LOCKED.zip')
RUNTIME=Path('/mnt/data/run_v101114_broad_runtime_matrix.py');SWRUN=Path('/mnt/data/run_v101114_sw_logic_matrix.js');INDEP=Path('/mnt/data/run_v101115_independent_prefreeze_audit.py')
...
html=base_html.replace("const APP_VERSION = 'v101.114';","const APP_VERSION = 'v101.115';",1).replace("const APP_EVIDENCE_STAGE = 'FOUR_PASS_AUDIT_EVIDENCE_INTEGRITY_REPAIR_R1';",f"const APP_EVIDENCE_STAGE = '{STAGE}';",1)
old="const BUILD_DATE = '2026-08-25'; // v101.114 / four-pass audit evidence-integrity repair";new="const BUILD_DATE = '2026-08-25'; // v101.115 / complete active-report line-audit closure";assert old in html;html=html.replace(old,new,1)
for n in PROT:assert ex(html,n)==BD[n],n
HTML_SHA=shab(html.encode())
qa=(BASE/'REAL_DEVICE_QA_CHECKLIST.md').read_text(encoding='utf-8').replace('v101.114','v101.115').replace('FOUR_PASS_AUDIT_EVIDENCE_INTEGRITY_REPAIR_R1',STAGE).replace('luisa-24h-v101-114',CACHE)
...
def prepare(t):
 ...
 wt(t/'reports/report_claims_vs_evidence_audit.md',f'''# Report claims vs evidence — {VERSION}\n\nThe active source-report inventory is stored in `metadata/active_report_inventory.json`. Every nonblank line of every listed source report is represented in `reports/active_report_line_audit.csv`, generated only after all listed reports exist. The audit CSV is the sole self-excluded audit output and is checked independently after ZIP freeze. Broad Chromium and isolated service-worker logic PASS claims bind to packaged prefreeze JSON evidence. Physical-device, live-origin PWA/offline and screen-reader tests remain `NOT_TESTED`; final reopened-ZIP PASS is not claimed inside the package.\n''')
 # current scripts
 shutil.copy2(RUNTIME,t/'scripts/run_broad_runtime_matrix.py');shutil.copy2(SWRUN,t/'scripts/run_sw_logic_matrix.js');shutil.copy2(INDEP,t/'scripts/run_independent_prefreeze_audit.py');shutil.copy2(Path(__file__),t/'scripts/build_v101115_active_report_line_closure.py')
...
ACTIVE={'README.md','REAL_DEVICE_QA_CHECKLIST.md','REAL_DEVICE_QA_RESULTS_TEMPLATE.csv','version.json','manifest.json','sw.js','metadata/build_provenance.json','evidence/v101115/PREPACKAGE_STAGE_REPORT.md','reports/full_regression_matrix.csv','reports/no_regression_fix_ledger.csv','reports/report_claims_vs_evidence_audit.md','reports/root_deploy_consistency_report.md','reports/nested_zip_consistency_report.md','reports/javascript_syntax_check.json','reports/service_worker_syntax_check.json'}
def allow(rel,tok):
 if tok=='v101.114':return rel in {'README.md','metadata/build_provenance.json','evidence/v101115/PREPACKAGE_STAGE_REPORT.md','reports/full_regression_matrix.csv','reports/no_regression_fix_ledger.csv'}
 if tok=='v101.113':return rel=='reports/no_regression_fix_ledger.csv'
 if tok=='v101.112':return rel=='reports/no_regression_fix_ledger.csv'
 return False
def stale(t):
 toks=['v101.114','v101.113','v101.112','v101.111','luisa-24h-v101-114','luisa-24h-v101-113','luisa-24h-v101-112','luisa-24h-v101-111','L24H_v101114_','L24H_v101113_','L24H_v101112_','L24H_v101111_'];rows=[];fail=0
 for p in fs(t):
  ...
  for tok in toks:
   ...
   if rel in ACTIVE:cls='BASELINE_PROVENANCE_ALLOWED' if allow(rel,tok) else 'FAIL_CURRENT_FACING_STALE'
...
for t in [A,B]:
 # runner currently expects evidence/v101115 and is otherwise parameterized
...
def lineaudit(t):
 ...
 for rel in ACTIVE_REPORTS:
  ...
  for ln,line in enumerate(p.read_text(encoding='utf-8',errors='ignore').splitlines(),1):
   ...
   if 'v101.114' in s:typ='CLAIM';bind='BASELINE_PROVENANCE_SHA'
   if 'v101.113' in s or 'v101.112' in s:
