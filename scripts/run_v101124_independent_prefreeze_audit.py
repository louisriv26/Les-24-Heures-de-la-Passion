...
VER='v101.124';STAGE='CROSS_RECORD_CONTINUITY_INLINE_FLOW_CLOSURE_R1';BASE_SHA='2959dfb832521af6f810d5f6b30ee187448aa799900993fd54ceae2b7c252b0d';CACHE='luisa-24h-v101-124'
...
par=json.loads((ROOT/'evidence/v101124/PROTECTED_DECLARATION_PARITY.json').read_text());ck('protected_13_plus_authorized_continuity_delta',par['unchanged']==13 and par['authorized_delta']==1,{'unchanged':par['unchanged'],'delta':par['authorized_delta']})
...
td=Path(tempfile.mkdtemp(prefix='v101124_prefreeze_'))
...
rr('full_package_builder_reproduction',['python',str(ROOT/'scripts/build_v101124_full_package_reconciliation.py'),str(BASE),str(td/'reb'),str(ROOT)],lambda:(True,{'status':'executed'}),60)
rr('continuity_candidate',['python',str(ROOT/'scripts/run_v101124_continuity_candidate_audit.py'),str(ROOT/'index.html'),str(td/'cand.json')],lambda:(json.loads((td/'cand.json').read_text())['status']=='PASS',json.loads((td/'cand.json').read_text())['status']))
rr('continuity_215',['python',str(ROOT/'scripts/run_v101124_continuity_matrix.py'),str(ROOT/'index.html'),str(td/'cont.json')],lambda:(json.loads((td/'cont.json').read_text())['pass']==215 and json.loads((td/'cont.json').read_text())['fail']==0,{'pass':json.loads((td/'cont.json').read_text())['pass'],'fail':json.loads((td/'cont.json').read_text())['fail'],'total':215}),60)
rr('ind_continuity_205',['python',str(ROOT/'scripts/run_v101124_independent_continuity_probe.py'),str(ROOT/'index.html'),str(td/'icont.json'),VER],lambda:(json.loads((td/'icont.json').read_text())['summary']=={'pass':205,'fail':0,'total':205},json.loads((td/'icont.json').read_text())['summary']),75)
rr('continuity_mutation_9',['python',str(ROOT/'scripts/run_v101124_continuity_mutation_tests.py'),str(ROOT/'index.html'),str(td/'cmut.json')],lambda:(json.loads((td/'cmut.json').read_text())['summary']=={'pass':9,'fail':0,'total':9},json.loads((td/'cmut.json').read_text())['summary']))
rr('hour24_state_16',['python',str(ROOT/'scripts/run_v101124_hour24_state_matrix.py'),str(ROOT/'index.html'),str(td/'state.json'),VER],lambda:(json.loads((td/'state.json').read_text())['summary']=={'pass':16,'fail':0,'total':16},json.loads((td/'state.json').read_text())['summary']))
rr('hour24_ux_70',['python',str(ROOT/'scripts/run_v101124_five_profile_ux_matrix.py'),str(ROOT/'index.html'),str(td/'ux.json'),VER],lambda:(json.loads((td/'ux.json').read_text())['summary']['pass']==70 and json.loads((td/'ux.json').read_text())['summary']['fail']==0,json.loads((td/'ux.json').read_text())['summary']),60)
rr('help_70',['python',str(ROOT/'scripts/run_v101124_help_browser_matrix.py'),str(ROOT/'index.html'),str(td/'help.json'),VER],lambda:(json.loads((td/'help.json').read_text())['summary']['pass']==70 and json.loads((td/'help.json').read_text())['summary']['fail']==0,json.loads((td/'help.json').read_text())['summary']),60)
...
rr('hour24_mutation_12',['python',str(ROOT/'scripts/run_v101124_mutation_tests.py'),str(ROOT/'index.html'),str(td/'hmut.json')],lambda:(json.loads((td/'hmut.json').read_text())['summary']=={'pass':12,'fail':0,'total':12},json.loads((td/'hmut.json').read_text())['summary']))
rr('ind_hour24_55',['python',str(ROOT/'scripts/run_v101124_independent_hour24_probe.py'),str(ROOT/'index.html'),str(td/'ih.json'),VER],lambda:(json.loads((td/'ih.json').read_text())['summary']['pass']==55 and json.loads((td/'ih.json').read_text())['summary']['fail']==0,json.loads((td/'ih.json').read_text())['summary']))
rr('ind_runtime_50',['python',str(ROOT/'scripts/run_v101124_independent_runtime_smoke.py'),str(ROOT/'index.html'),str(td/'ir.json'),VER],lambda:(json.loads((td/'ir.json').read_text())['summary']['pass']==50 and json.loads((td/'ir.json').read_text())['summary']['fail']==0,json.loads((td/'ir.json').read_text())['summary']))
rr('ind_presentation_1990',['python',str(ROOT/'scripts/run_v101124_independent_presentation_matrix.py'),str(ROOT/'index.html'),str(ledger),str(td/'ip.json'),VER],lambda:(json.loads((td/'ip.json').read_text())['summary']['pass']==1990 and json.loads((td/'ip.json').read_text())['summary']['fail']==0,json.loads((td/'ip.json').read_text())['summary']),90)
rr('stale_scans',['python',str(ROOT/'scripts/run_v101124_stale_scans.py'),str(ROOT),str(td/'vscan.json'),str(td/'sscan.json')],lambda:(json.loads((td/'vscan.json').read_text())['status']=='PASS' and json.loads((td/'sscan.json').read_text())['status']=='PASS',{'version':json.loads((td/'vscan.json').read_text())['status'],'semantic':json.loads((td/'sscan.json').read_text())['status']}))
status='PASS_PREFREEZE_INDEPENDENT_FOUR_PASS' if all(x['status']=='PASS' for x in C) else 'FAIL';obj={'schema':'L24H_V101124_INDEPENDENT_PREFREEZE_V1','version':VER,'stage':STAGE,'status':status,'checks_pass':sum(x['status']=='PASS' for x in C),'checks_total':len(C),'checks_fail':sum(x['status']=='FAIL' for x in C),'checks':C}
OJ.parent.mkdir(parents=True,exist_ok=True);OJ.write_text(json.dumps(obj,indent=2)+'\n');OM.write_text('# Independent prefreeze audit — v101.124\n\n**'+status+'**\n\n'+'\n'.join(f'- `{x["check"]}` — **{x["status"]}** — `{str(x["detail"])[:800]}`' for x in C)+'\n');shutil.rmtree(td,ignore_errors=True);print(json.dumps({'status':status,'pass':obj['checks_pass'],'fail':obj['checks_fail'],'total':obj['checks_total']}));raise SystemExit(0 if status.startswith('PASS') else 2)
