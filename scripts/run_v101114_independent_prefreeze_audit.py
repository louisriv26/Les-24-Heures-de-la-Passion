...
qa=(TREE/'REAL_DEVICE_QA_CHECKLIST.md').read_text();qt=(TREE/'REAL_DEVICE_QA_RESULTS_TEMPLATE.csv').read_text();ck('qa_checklist_current',VERSION in qa and 'v101.113' not in qa.splitlines()[0]);qrows=list(csv.DictReader(qt.splitlines()));ck('qa_template_21',len(qrows)==21 and all(r['app_version']==VERSION for r in qrows),len(qrows))
r=json.loads((TREE/'evidence/v101114/BROAD_CHROMIUM_RUNTIME_MATRIX.json').read_text());ck('broad_runtime',r['summary']=={'pass':52,'fail':0,'total':52},r['summary'])
s=json.loads((TREE/'evidence/v101114/SERVICE_WORKER_LOGIC_MATRIX.json').read_text());ck('sw_logic',s['summary']=={'pass':15,'fail':0,'total':15},s['summary'])
