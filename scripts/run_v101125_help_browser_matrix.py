...
HTML=Path(sys.argv[1]).read_text(encoding='utf-8'); OUT=Path(sys.argv[2]); VERSION='v101.125'; STAGE='FOUR_PASS_EVIDENCE_SCHEMA_AND_DIRECT_REPORT_BINDING_RECONCILIATION_R1'
profiles=[
...
async def main():
 ...
 OUT.write_text(json.dumps({'schema':'L24H_V101125_HELP_BROWSER_MATRIX_V1','version':VERSION,'browser':'system Chromium via Playwright content injection','real_device_limitation':'Not physical Safari/Samsung/TalkBack/VoiceOver evidence','summary':summary,'results':rows},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print(json.dumps(summary),flush=True)
 if summary['fail']:
  for r in rows:
   if r['status']=='FAIL': print('FAIL',r,flush=True)
 sys.stdout.flush(); sys.stderr.flush(); os._exit(2 if summary['fail'] else 0)
asyncio.run(main())
