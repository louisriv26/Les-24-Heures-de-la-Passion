...
async def main():
 ...
 async with async_playwright() as pw:
  ...
  for pn,w,h in PROFILES:
   ...
   await ctx.close()
  await b.close()
 out={'schema':'L24H_V101125_INDEPENDENT_CONTINUITY_PROBE_V1','version':VER,'summary':{'pass':sum(r['status']=='PASS' for r in rows),'fail':sum(r['status']=='FAIL' for r in rows),'total':len(rows)},'rows':rows};OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n');print(json.dumps(out['summary']));raise SystemExit(0 if out['summary']['fail']==0 else 2)
asyncio.run(main())
