...
HTML=Path(sys.argv[1]).read_text(encoding='utf-8');OUT=Path(sys.argv[2]);VER=sys.argv[3] if len(sys.argv)>3 else 'v101.122'
...
async def main():
 ...
 OUT.write_text(json.dumps({'schema':'L24H_V101122_INDEPENDENT_HOUR24_PROBE_V1','version':VER,'implementation':'separate browser probe; does not call primary Hour-24 matrix','summary':summary,'rows':rows},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
