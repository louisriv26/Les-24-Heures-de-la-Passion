...
td=Path(tempfile.mkdtemp(prefix='v101125_primary_reopen_'));root=td/'root';root.mkdir()
try:
 ...
 html=(root/'index.html').read_text();ck('current_identity',"const APP_VERSION = 'v101.125';" in html and "const APP_EVIDENCE_STAGE = 'FOUR_PASS_EVIDENCE_SCHEMA_AND_DIRECT_REPORT_BINDING_RECONCILIATION_R1';" in html)
 # manifest/hash exact
 ...
 pj=td/'prefreeze.json';pmf=td/'prefreeze.md';p=subprocess.run(['python',str(root/'scripts/run_v101125_independent_prefreeze_audit.py'),str(BASE),str(root),str(pj),str(pmf)],capture_output=True,text=True,timeout=1800);po=json.loads(pj.read_text()) if pj.exists() else {};ck('fresh_prefreeze',p.returncode==0 and po.get('status')=='PASS_PREFREEZE_INDEPENDENT_FOUR_PASS',{'returncode':p.returncode,'summary':{k:po.get(k) for k in ['checks_pass','checks_fail','checks_total']},'stderr':p.stderr[-500:]})
 # exact full builder + deterministic rebuild
 rebuilt=td/'rebuilt';rz=td/'rebuilt.zip';p=subprocess.run(['python',str(root/'scripts/build_v101125_full_package_reconciliation.py'),str(BASE),str(rebuilt),str(root),str(rz)],capture_output=True,text=True,timeout=300);ck('full_builder_reproduction',p.returncode==0,{'returncode':p.returncode,'stderr':p.stderr[-500:]});ck('deterministic_zip_rebuild',p.returncode==0 and sha(rz)==sha(ZIP),{'frozen':sha(ZIP),'rebuilt':sha(rz) if rz.exists() else None})
 ...
 rs=json.loads((root/'evidence/v101125/ACTIVE_REPORT_LINE_AUDIT_SUMMARY.json').read_text());ra=json.loads((root/'evidence/v101125/REPORT_CLAIM_ASSERTIONS.json').read_text());ck('claim_specific_report_bindings',rs.get('status')=='PASS' and rs.get('exact_coverage') is True and rs.get('nonblank_lines')==len(exp) and ra.get('status')=='PASS' and ra.get('assertions_total')==len(exp) and ra.get('assertions_fail')==0,{'summary':rs,'assertions_total':ra.get('assertions_total'),'assertions_fail':ra.get('assertions_fail')})
 status='PASS' if all(x['status']=='PASS' for x in C) else 'FAIL';obj={'schema':'L24H_V101125_PRIMARY_FINAL_REOPEN_V1','version':'v101.125','status':status,'zip_sha256':sha(ZIP),'html_sha256':sha(root/'index.html'),'checks_pass':sum(x['status']=='PASS' for x in C),'checks_total':len(C),'checks':C};OJ.parent.mkdir(parents=True,exist_ok=True);OJ.write_text(json.dumps(obj,indent=2)+'\n');OM.write_text('# Primary final reopened-ZIP audit — v101.125\n\n**'+status+'**\n\n'+'\n'.join(f'- `{x["check"]}` — **{x["status"]}** — `{str(x["detail"])[:800]}`' for x in C)+'\n');print(json.dumps({'status':status,'pass':obj['checks_pass'],'total':obj['checks_total'],'zip_sha256':obj['zip_sha256']}));raise SystemExit(0 if status=='PASS' else 2)
