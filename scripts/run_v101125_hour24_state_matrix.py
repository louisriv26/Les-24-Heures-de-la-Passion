...
HTML=Path(sys.argv[1]).read_text(encoding='utf-8'); OUT=Path(sys.argv[2]); VERSION=sys.argv[3] if len(sys.argv)>3 else 'v101.125'
...
async def main():
 ...
 OUT.write_text(json.dumps({'schema':'L24H_V101125_HOUR24_STATE_TRANSITION_MATRIX_V1','version':VERSION,'browser':'system Chromium via Playwright content injection','real_device_limitation':'browser emulation only; physical-device gates not claimed','summary':summary,'rows':rows},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
