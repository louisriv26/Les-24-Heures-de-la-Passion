...
HTML=Path(sys.argv[1]).read_text(encoding='utf-8'); OUT=Path(sys.argv[2]); VERSION=sys.argv[3] if len(sys.argv)>3 else 'v101.128'
EXPECTED=[
...
async def main():
 ...
 async with async_playwright() as pw:
  ...
  for label,w,h in profiles:
   ...
   await page.close()
  await browser.close()
 OUT.parent.mkdir(parents=True,exist_ok=True); OUT.write_text(json.dumps({'schema':'L24H_V101128_LEGACY_CONTINUITY_MATRIX_V1','version':VERSION,'rows':rows,'pass':sum(r['status']=='PASS' for r in rows),'fail':sum(r['status']=='FAIL' for r in rows)},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
