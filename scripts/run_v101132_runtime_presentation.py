...
async def main():
 ...
 async with async_playwright() as pw:
  ...
  ident=await p.evaluate("()=>({v:APP_VERSION,s:APP_EVIDENCE_STAGE})");add('identity',ident=={'v':'v101.132','s':'DEEP_FOUR_PASS_RELEASE_ENGINEERING_RECONCILIATION_R1'},ident)
 ...
 sm={'pass':sum(r['status']=='PASS' for r in rows),'fail':sum(r['status']=='FAIL' for r in rows),'total':len(rows)};OUT.write_text(json.dumps({'schema':'L24H_V101132_RUNTIME_PRESENTATION_MATRIX_V1','version':'v101.132','summary':sm,'rows':rows},ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps(sm));
 if sm['fail']:
