...
sm={'pass':sum(r['status']=='PASS' for r in rows),'fail':sum(r['status']=='FAIL' for r in rows),'total':len(rows)};OUT.write_text(json.dumps({'schema':'L24H_V101135_GLOBAL_RAW_QUOTE_MUTATION_TESTS_V1','version':'v101.135','summary':sm,'rows':rows},ensure_ascii=False,indent=2)+'\n');print(json.dumps(sm));raise SystemExit(2 if sm['fail'] else 0)
