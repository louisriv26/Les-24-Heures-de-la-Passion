...
HTML=Path(sys.argv[1]).read_text(encoding='utf-8'); OUT=Path(sys.argv[2]); VERSION=sys.argv[3]; STAGE=sys.argv[4]
profiles=[
...
async def main():
 ...
 async with async_playwright() as pw:
  ...
  for label,w,h,ua in profiles:
   ...
   await page.set_viewport_size({'width':w,'height':h}); await page.wait_for_timeout(40)
   await page.evaluate("() => { try { if(document.getElementById('helpModalOverlay')) closeHelpModal(); } catch(e){} let inv=document.getElementById('__helpTestInvoker'); if(!inv){inv=document.createElement('button');inv.id='__helpTestInvoker';inv.textContent='Aide test';document.body.appendChild(inv);} inv.focus(); }")
   base=await page.evaluate("() => ({version:APP_VERSION,stage:APP_EVIDENCE_STAGE,ua:navigator.userAgent})")
   add(label,'version_stage',base['version']==VERSION and base['stage']==STAGE,base)
   opened=await page.evaluate('''() => { try { showHelp(); const o=document.getElementById('helpModalOverlay'); const m=o&&o.querySelector('.help-modal'); const sc=o&&o.querySelector('.help-modal-scroll'); const secs=o?[...o.querySelectorAll('.help-section')]:[]; const qs=o?[...o.querySelectorAll('.help-quick-btn')]:[]; return {ok:!!o&&!!m&&!!sc,role:o&&o.getAttribute('role'),modal:o&&o.getAttribute('aria-modal'),sections:secs.length,rows:o?o.querySelectorAll('.help-row').length:0,quick:qs.length,sectionIds:secs.map(x=>x.id),quickText:qs.map(x=>x.innerText.trim()),text:o?o.innerText:'',hOverflow:sc?sc.scrollWidth-sc.clientWidth:null}; } catch(e){return {ok:false,error:String(e)}} }''')
   ...
   await page.evaluate("() => closeHelpModal()"); await page.wait_for_timeout(30)
 ...
 OUT.write_text(json.dumps({'schema':'L24H_V101134_HELP_BROWSER_MATRIX_OPTIMIZED_V1','version':VERSION,'browser':'system Chromium via Playwright single content injection; five viewport/user-agent passes','equivalence_note':'Same 14 assertions per profile as inherited v101.128 help matrix; optimization removes repeated full-HTML parsing only.','real_device_limitation':'Not physical Safari/Samsung/TalkBack/VoiceOver evidence','summary':summary,'results':rows},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print(json.dumps(summary),flush=True)
 if summary['fail']:
  for r in rows:
   if r['status']=='FAIL': print('FAIL',r,flush=True)
 await asyncio.sleep(0); os._exit(2 if summary['fail'] else 0)
asyncio.run(main())
